...
# paths
datapath = "/raid/work/yamato/edisk_data/L1489IRS/"
...
def get_image_basename(source, baseline, line, robust=0.5, dv=None, other=None, type="image"):
    ...
    source : str
        Source name
    baseline : str
        Configuration index, basically ``SB`` or ``SBLB``
    line : str
        line name (or ``continuum``)
    robust : float, optional
        Robust parameter, by default 0.5
    type : str, optional
        image type, basicallly either ``image``, ``mask``, ``pb``, or ``pbcor``, by default "image"
...
data_scaling_factor = {"cont": 1e3,
                       "M0": 1.0,
                       "M1": 1e-3,
                       "M8": 1.0,}
beam_kw = {"cont": dict(fill=True, color="white"), 
           "M0": dict(fill=True, color="white"),
           "M1": dict(fill=True, color="black"),
           "M8": dict(fill=True, color="white")}
cbar_kw = {"cont": dict(label=r"mJy beam$^{-1}$"), 
           "M0": dict(label=r"mJy beam$^{-1}$ km s$^{-1}$"),
...
cmap_kw = {"cont": dict(cmap="inferno")}

class ContinuumNormalize(ImageNormalize):

    def __init__(self, data, interval=None, vmin=0.0, vmax=None, stretch=AsinhStretch(a=0.02), clip=False, invalid=-1):
        super().__init__(data, interval, vmin, vmax, stretch, clip, invalid)
...
def get_casa_ellipse_region(dir, maj, min, pa):
    ...
    region = "ellipse[[%s, %s], [%.1farcsec, %.1farcsec], %.1fdeg]" % (ra, dec, maj, min, pa)
...
#     image_mask = np.ones(data.shape[1:])
#     image_mask[r > r_out] = 0.0

#     nchan = data.shape[0]
#     channels = np.arange(nchan)
#     firstchannel, lastchannel = list(map(lambda x:int(x), emission_channel.split("~")))
#     channel_mask = np.ones(nchan)

#     if excludechan:
#         channel_mask[np.logical_and(channels >= firstchannel, channels <= lastchannel)] = 0.0
#     else:
#         channel_mask[np.logical_or(channels < firstchannel, channels > lastchannel)] = 0.0

#     mask = np.logical_and(image_mask[None, :, :], channel_mask[:, None, None])
...
#     nchan = np.sum(masked_data != 0.0, axis=0)

#     if dchan.size != 1:
#         raise ValueError("Velocity axis not equally spaced.")
...
#     #beam = fetch_beam_info(fitsfile, pa_rotate=True)
