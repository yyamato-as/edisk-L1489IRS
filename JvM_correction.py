...
def get_JvM_epsilon(psfname, ntheta=24):
    ...
    # add a new axis in case of 2D image to make easier doing in systemic manner
    if psf_array.ndim < 3:
        psf_array = np.expand_dims(psf_array, axis=2)

    # determine the null points in each azimuthal direction
    ref_chan_ind = int(psf_array.shape[2]/2) # assume psf is same along the spectral axis and use the center channel; sometimes edge channel have not a good representation of psf over the channels? 
    nulled_psf = get_nulled_psf(psf_array[:, :, ref_chan_ind], r, theta, ntheta=ntheta)
...
def apply_JvM_correction(prefix, mfs=True, ntheta=24, pbcor=True):
    ...
    to_export = [image_JvMcorr]

    print(f"JvM-corrected image is stored in {image_JvMcorr}.")

    if pbcor:
        image_JvMcorr_pbcor = image_JvMcorr.replace(".image", ".pbcor")
        pbimage = prefix + ".pb.tt0" if mfs else prefix + ".pb"
        if os.path.exists(image_JvMcorr_pbcor):
            shutil.rmtree(image_JvMcorr_pbcor)
        casatasks.impbcor(
            imagename=image_JvMcorr, pbimage=pbimage, outfile=image_JvMcorr_pbcor
        )
        to_export.append(image_JvMcorr_pbcor)
...
if __name__ == '__main__':
    ...
    prefix = "./test_JvM_effect/L1489IRS_SB_continuum_robust_0.5"

    apply_JvM_correction(prefix, ntheta=24, mfs=True)