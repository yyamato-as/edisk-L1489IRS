...
linelabel = {"12CO": "$^{12}$CO ($J=$ 2--1)", "13CO": "$^{13}$CO ($J=$ 2--1)", "C18O": "C$^{18}$O ($J=$ 2--1)", "SO": "SO ($J_N=$ $6_5$--$5_4$)"}
linelabel_noqn = {"12CO": "$^{12}$CO", "13CO": "$^{13}$CO", "C18O": "C$^{18}$O", "SO": "SO"}

def plot_rotation_map(mapname, cubename, center_coord=None, xlim=None, ylim=None, vrange=5, vsys=0.0, threshold=4, deltav=1.5, figsize=(3,3)):
    rotmap = FitsImage(mapname)
    rotmap.shift_phasecenter_toward(center_coord)
    # rotmap.get_disk_coord(center_coord=center_coord)

    if not "k" in rotmap.data_unit:
        rotmap.data *= 1e-3 # in km/s
    ...
    cube.get_threshold_mask(threshold=threshold)

    threshold_mask = np.sum(cube.threshold_mask, axis=0) >= 1.0

    # velocity range mask
    vrange_mask = (rotmap.data <= vsys + deltav) & (rotmap.data >= vsys - deltav)
    ...
    utils.plot_2D_map(
        ...
        contour=False,
        cmap_kw=dict(cmap=cmap["M1"], vmin=vsys-vrange, vmax=vsys+vrange),
        cbar_kw=dict(label="km s$^{-1}$"),
        xlim=xlim,
        ylim=ylim,
    )
...
def plot_map(
    ...
    utils.plot_2D_map(
        ...
        scale=scale,
        cmap_kw=dict(cmap=cmap, norm=norm),
        # cmap_kw=dict(levels=50, cmap=cmap),
        cbar_kw=cbar_kw,
        beam_kw=beam_kw,
        sbar_kw=sbar_kw,
    )
