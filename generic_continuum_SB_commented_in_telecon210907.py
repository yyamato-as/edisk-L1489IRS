...
field   = 'fieldName' # field name from listobs task ###if the field name have any spaces, put double quote within the single quote
prefix  = 'filename prefix' # same as field

### always include trailing slashes!!
WD_path = '/lustre/cv/projects/edisk/sourceDirectory/' # change to my own working directory
SB_path = WD_path+'SB/'
LB_path = WD_path+'LB/'
...
pl_data_params={'SB1': {'vis': SB_path+'uid___A002_Xeb6c00_X22e.ms', # change for specific dataset
...
data_params = {'SB1': {'vis' : WD_path+prefix+'_SB1.ms',
                       ...
                       'orig_spw_map': {25:0, 27:1, 29:2, 31:3, 33:4, 35:5, 37:6},  # mapping of old spws to new spws (needed for cont.dat to work) # need to swap the 27 and 31 for DDT data
                       ...
                       'phasecenter': '',
                       'timerange': '2021/04/24/01:45:00~2021/04/24/03:00:00', # from listobs: obs start time to obs finish time: no need to set an exactly one, near value is ok
                       'contdotdat' : 'SB/cont.dat' # some archival data don't have cont.dat; don't need to fill in that case
                      }, 
               }
...
if not skip_plots:
    ...
    plot_deprojected(export_vislist,
                     ...
                     show_err=False)                                           # check if there are any flux offset between the different datasets; actual difference or decorrelation
...
###############################################################
selectedVis='vis_avg_rescaled' # usually use unshifted MS; to avoid the side effect to the seflcal
...
SB_spwmap=[0,0,0,0,0,0,0] # should be all 0; the number of 0 should be number of spw
SB_contspws = '' 
...
### copy the message on the terminla here ###
...
self_calibrate(prefix,data_params,selectedVis,mode='SB-only',iteration=iteration,selfcalmode='p',nsigma=50.0,solint='inf',
               ...
               SB_contspws=SB_contspws,SB_spwmap=SB_spwmap,parallel=parallel)                  # nsigma should be around half of peak SNR
...
self_calibrate(prefix,data_params,selectedVis,mode='SB-only',iteration=iteration,selfcalmode='p',nsigma=25.0,solint='30s',  
               noisemasks=[common_mask,noise_annulus],
               SB_contspws=SB_contspws,SB_spwmap=SB_spwmap,parallel=parallel)          # solint should be around 1/2 of scan length (can be checked with listobs)
