...
def LSRKvel_to_chan(msfile, field, spw, restfreq, LSRKvelocity):
    ...
    chanvelocities = (
        (restfreq - lsrkfreqs) / restfreq * cc / 1.0e3
...
def flagchannels_from_contdotdat(ms_dict):
    ...
    for j, spw in enumerate(contdotdat):
        ...
        for k in range(contdotdat[spw].shape[0]):
            ...
            chans = np.concatenate(
                (
                    LSRKfreq_to_chan(
                        ms_dict["vis"], ms_dict["field"], spw, contdotdat[spw][k]
                    ),
                    chans,
                )
            )
    ...
    print(
        "# Flagchannels input string for %s from cont.dat file: '%s'"
...
def get_flagchannels(ms_dict, output_prefix):
        ...
        for count, j in enumerate(np.where(ms_dict["line_spws"] == spw)[0]):
            ...
            if count == 0:
                chans = LSRKvel_to_chan(
                    ...
                    ms_dict["flagrange"][j],
                ).reshape((1, ms_dict["flagrange"][j].size))
    ...
    if "contdotdat" in ms_dict:
        ...
        cont_dict = dict(
            [line.split(":") for line in contdot_dat_flagchannels_string.split()]
        )
        ...
        keys = np.unique(
            np.concatenate(
                (np.array(list(basic_dict.keys())), np.array(list(cont_dict.keys())))
            )
        )
        merged_dict = {}
        for key in keys:
            if key in basic_dict and key not in cont_dict:
                ...
                merged_dict[key] = numpy.concatenate(
                    (basic_dict[key], cont_dict[key]), axis=0
                )
                ...
                chans = list(
                    merged_dict[key][merged_dict[key][:, 0].argsort(), :].flatten()
                )
...
def get_contsub_spws(ms_dict, output_prefix):
    ...
    for j, spw in enumerate(ms_dict["cont_spws"]):
        if flagchannels_string != "":
            ...
            flagchannels_string += "%d" % (spw)
    print(
        "# Contsub spw selection for %s: '%s'" % (ms_dict["name"], flagchannels_string)
    )
    return flagchannels_string
...
def avg_cont(
    ...
    if width_array is None and contspws is None:
        width_array = (
            ...
            .astype("int")
            .tolist()
        )  # array of number of channels to average to form an output channel (to be passed to mstransform)
        ...
        print(
            "If either contspws or width_array is set to a value, the other parameter has to be manually set as well"
        )
        return
    ...
    if len(flagchannels) == 0:
        ...
        if os.path.isdir(msfile + ".flagversions/flags.before_cont_flags"):
            flagmanager(
                vis=msfile, mode="delete", versionname="before_cont_flags"
            )  # clear out old versions of the flags
        if os.path.isdir(msfile + ".flagversions/flags.cont_flags"):
            flagmanager(
                vis=msfile, mode="delete", versionname="cont_flags"
            )  # clear out old versions of the flags
        ...
        flagmanager(
            ...
            comment="Flag state after line flagging",
        )
        outputvis = output_prefix + "_" + ms_dict["name"] + "_initcont.ms"
        os.system("rm -rf " + outputvis)
        split(
        ...
        flagmanager(
            vis=msfile, mode="restore", versionname="before_cont_flags"
...
def contsub(
    msfile, output_prefix, spw="", flagchannels="", datacolumn="data", excludechans=True
):
    """
    Produce spectrally averaged continuum measurement sets
...
def image_each_obs(
    ...
    tb.open(msfile + "/OBSERVATION")
    num_observations = (tb.getcol("TIME_RANGE")).shape[
        1
...
def fit_gaussian_pc(imagename, region, dooff=False, mask=""):
    ...
    if coordsystem == "J2000":
        ...
        return (
            au.convertColonDelimitersToHMSDMS(J2000coords).replace(",", ""),
            peak_ra_deg,
            peak_dec_deg,
            phasecenter_ra_deg,
            phasecenter_dec_deg,
...
def split_all_obs(msfile, nametemplate):
    ...
    for i in obs_ids:
        ...
        fields = np.unique(
            field_col[np.where(obs_col == i)]
        )  # sometimes the MS secretly has multiple field IDs lurking even if listobs only shows one field
        if len(spws) == 1:
            spw = str(spws[0])
        else:
...
def estimate_flux_scale(
    ...
    print(
        "#The error on the weighted mean ratio is %.3e, although it's likely that the weights in the measurement sets are off by some constant factor"
        % (1 / np.sqrt(np.sum(w)),)
    )
    plt.figure()
    plt.errorbar(
...
def self_calibrate(
    ...
    imsize=None,
    scales=None,
    finalimageonly=False,
    remove_all_following_iterations=True,
    ...
    spwmap = [0, 0, 0, 0, 0]
    if (
        remove_all_following_iterations
    ):  ### Remove all following selfcal files after the iteration being run
        if (
            not skipImage
        ):  # don't erase the current iteration if you want to skip imaging and re-run with current model
            os.system(
                "rm -rf " + "*_" + mode + "*p{{{0:d}..99}}*".format(iteration + 1)
            )
        else:
    ...
    for i in data_params.keys():
        ...
        if (mode == "LB-only") and ("LB" in i):  # skip over SB EBs if in LB-only mode
            continue
        if (
            (mode == "LB+SB") and ("SB" in i) and (iteration == 0)
        ):  # slightly different flow for the SB data in iter0
            if not skipImage:
                os.system(
                    "rm -rf "
                    + data_params[i][selectedVis].replace(".ms", "_" + mode + "_p0.ms")
                )
                os.system(
                    "cp -r "
                ...
                data_params[i]["vis_avg_selfcal"] = data_params[i][selectedVis].replace(
                    ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".ms"
                )
            vislist.append(
                data_params[i][selectedVis].replace(
                    ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".ms"
                )
            )
            continue
        if iteration == 0:
            ...
            if (
                len(data_params[i]["selfcal_tables"]) > iteration
            ):  ### remove selfcal table entries when redoing self-cal stages
                data_params[i]["selfcal_tables"] = data_params[i]["selfcal_tables"][
                    0:iteration
                ]
                data_params[i]["selfcal_spwmap"] = data_params[i]["selfcal_spwmap"][
                    0:iteration
                ]
            if not skipImage:
                print(
                    ...
                    data_params[i][selectedVis].replace(
                        ".ms",
                        "_" + mode + "_" + prevselfcalmode + str(iteration - 1) + ".ms",
                    ),
                )
                os.system(
                    "rm -rf "
                ...
                split(
                    vis=data_params[i][selectedVis].replace(
                        ".ms",
                        "_" + mode + "_" + prevselfcalmode + str(iteration - 1) + ".ms",
                    ),
                    outputvis=data_params[i][selectedVis].replace(
                        ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".ms"
                    ),
                )
                data_params[i]["vis_avg_selfcal"] = data_params[i][selectedVis].replace(
    ...
    if not skipImage:
        ...
        tclean_wrapper(
            vis=vislist,
            imagename=prefix + "_" + mode + "_" + prevselfcalmode + str(iteration),
            scales=scales,
            nsigma=nsigma,
    ...
    if (
        finalimageonly == True
    ):  # break out of function if we just wanted final image and no more gain solutions
        estimate_SNR(
            prefix + "_" + mode + "_" + prevselfcalmode + str(iteration) + ".image.tt0",
            disk_mask=noisemasks[0],
    ...
    for i in data_params.keys():
        ...
        os.system(
            "rm -rf "
            + data_params[i][selectedVis].replace(
                ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".g"
            )
        )
        if "LB" in i:
        ...
        if selfcalmode == "p":
            gaincal(
                ...
                caltable=data_params[i][selectedVis].replace(
                    ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".g"
                ),
                gaintype="T",
                spw=contspws,
                refant=data_params[i]["refant"],
                calmode=selfcalmode,
                solint=solint,
                minsnr=minsnr,
                minblperant=4,
            ...
            plot_ants_flagging_colored(
                "plot_ants_"
                + data_params[i][selectedVis].replace(
                    ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".png"
                ),
                data_params[i][selectedVis].replace(
                    ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".ms"
                ),
        ...
        data_params[i]["selfcal_tables"].append(
            data_params[i][selectedVis].replace(
                ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".g"
            )
        )
    ...
    for i in data_params.keys():
        ...
        applycal(
            ...
            gaintable=data_params[i][selectedVis].replace(
                ".ms", "_" + mode + "_" + selfcalmode + str(iteration) + ".g"
            ),
            interp="linearPD",
            calwt=True,
        ...
        data_params[i]["selfcal_spwmap"].append(
            spwmap
        )  # Add spw map to the dictionary for use later
...
def get_solints_simple(vislist, scantimesdict, integrationtimes):
    ...
    for vis in vislist:
        ...
        for target in targets:
            all_integrations = np.append(
                all_integrations, integrationtimes[vis][target]
            )
    integration_time = np.max(
        all_integrations
    )  # use the longest integration time from all MS files
    ...
    while (
        ...
        if ints_per_solint.is_integer():
            ...
            remainder = ints_per_solint - float(
                int(ints_per_solint)
            )  # calculate delta_T greater than an a fixed multile of integrations
            solint = (
                solint - remainder * integration_time
            )  # add remainder to make solint a fixed number of integrations
        ...
        for target in targets:
            ...
            for key in keylist:
                if "scan" in key and listdict[vis][key]["0"]["FieldName"] == target:
                    countscans += 1
                    scantime = (
                        listdict[vis][key]["0"]["EndTime"]
                        - listdict[vis][key]["0"]["BeginTime"]
                    ) * 86400.0
                    ints_per_scan = np.round(
                        scantime / listdict[vis][key]["0"]["IntegrationTime"]
                    )
                    integrationtime = np.append(
                        integrationtime,
                        np.array([listdict[vis][key]["0"]["IntegrationTime"]]),
                    )
                    ...
                    min_spws = np.append(
                        np.min(listdict[vis][key]["0"]["SpwIds"]), min_spws
                    )
...
def test_truncated_scans(ints_per_solint, allscantimes, integration_time):
    ...
    for i in range(len(delta_ints_per_solint)):
        diff_ints_per_scan = (
            (
                allscantimes
                - ((ints_per_solint + delta_ints_per_solint[i]) * integration_time)
            )
            / integration_time
        ) + 0.5
        diff_ints_per_scan = diff_ints_per_scan.astype(int)
        trimmed_scans = (
            (diff_ints_per_scan > 0.0)
            & (diff_ints_per_scan < ints_per_solint + delta_ints_per_solint[i])
        ).nonzero()
        ...
        if (
            (i > 0)
            and (n_truncated_scans[i] <= n_truncated_scans[min_index])
            and (n_remaining_ints[i] < n_remaining_ints[min_index])
        ):
            min_index = i
...
def plot_ants_flagging_colored(filename, vis, gaintable):
    ...
    ax.scatter(
        offset_x[ants_zero_flagging[0]],
        offset_y[ants_zero_flagging[0]],
        marker="o",
        color="green",
        label="No Flagging",
        s=120,
    ...
    ax.scatter(
        offset_x[ants_lt10pct_flagging[0]],
        offset_y[ants_lt10pct_flagging[0]],
        marker="o",
        color="blue",
        label="<10% Flagging",
        s=120,
    ...
    ax.scatter(
        offset_x[ants_lt25pct_flagging[0]],
        offset_y[ants_lt25pct_flagging[0]],
        marker="o",
        color="yellow",
        label="<25% Flagging",
        s=120,
    ...
    ax.scatter(
        offset_x[ants_lt50pct_flagging[0]],
        offset_y[ants_lt50pct_flagging[0]],
        marker="o",
        color="magenta",
        label="<50% Flagging",
        s=120,
    ...
    ax.scatter(
        offset_x[ants_lt75pct_flagging[0]],
        offset_y[ants_lt75pct_flagging[0]],
        marker="o",
        color="cyan",
        label="<75% Flagging",
        s=120,
    ...
    ax.scatter(
        offset_x[ants_gt75pct_flagging[0]],
        offset_y[ants_gt75pct_flagging[0]],
        marker="o",
        color="black",
        label=">75% Flagging",
        s=120,
...
def plot_image(filename, outname, min=None, max=None):
    ...
    if min == None:
        imview(
            raster={"file": filename, "scaling": -1},
            zoom={
                "blc": [int(size / 4), int(size / 4)],
        ...
        imview(
            raster={"file": filename, "scaling": -1, "range": [min, max]},
            zoom={
                "blc": [int(size / 4), int(size / 4)],
                "trc": [int(size - size / 4), int(size - size / 4)],
            },
...
def get_flagged_solns_per_ant(gaintable, vis):
    ...
    offset_y = [(offset[i]["latitude offset"]["value"]) for i in range(len(names))]
    offset_x = [(offset[i]["longitude offset"]["value"]) for i in range(len(names))]
    # Calculate the number of flags for each antenna.
